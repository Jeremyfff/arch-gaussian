...
from RePaint.utils import debug_utils
import datetime
            ...
            for t_last, t_cur in time_pairs:
                ...
                if t_cur < t_last:  # reverse
                    with th.no_grad():
                        ...
                        image_after_step = out["sample"]  # [1, 3, 256, 256] torch.Tensor
                        pred_xstart = out["pred_xstart"]  # [1, 3, 256, 256] torch.Tensor
                        sample_idxs[t_cur] += 1

                        # region debug
                        if loop_count % 3 == 0:
                            debug_utils.save_images(image_after_step, pred_xstart,
                                                    "image_after_step", "pred_xstart",
                                                    f"./Repaint/log/debug/{time_str}/{loop_count}.png")
                        loop_count += 1
                        # endregion
