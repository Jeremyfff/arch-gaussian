...
uniform float _Detail; // value= 2.5, min=1., max=5., step=0.1


#define _CameraDist 3.

#define _Saturation 0.37
#define _Color0 vec3(180./255.,205./255.,245./255.) * 0.5
#define _Color1 vec3(173./255.,215./255.,252./255.) * 0.5
#define _Color2 vec3(202./255.,204./255.,235./255.) * 0.5
...
void main() {
  ...
  vec3 huv = 7. + vec3(uv, iTime * (_Frequency * TemporalFrequency / _Detail));
  ...
  vec3 col = getBgCol(rf - vec3(uv * _Detail, iTime * TemporalFrequency));

  vec3 rfr = refract(-rd, normal, 1./1.4);
  col += getBgCol(rfr +  vec3(uv * _Detail, iTime * TemporalFrequency));
