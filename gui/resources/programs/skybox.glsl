...
void main() {
    ...
    vec3 inv_wave_length = 1.0 / (1.0 - _WaveLengthColor);
    // Light diection
    vec3 v3LightDir = normalize(vec3(-_LightDir.x, -_LightDir.z, _LightDir.y));
    // Ray initialization
    //vec2 v2uv = fragCoord.xy / iResolution.xy;
    vec2 v2uv = vec2(uv0.x, uv0.y);
    ...
    if (isGround)
    {
        ...
        fragColor = vec4(1.0 - exp(-(v3RayleighColor + _GroundColor * v3MieColor)), 1.0);
